...
class Edit extends \Magento\Backend\Block\Widget\Form\Container
{
    ...
    protected function _construct()
    {
        ...
        if (!$this->_authorization->isAllowed('Licentia_Panda::tags_delete')) {
            $this->buttonList->remove('delete');
        }

        $this->buttonList->remove('save');

        if ($this->_authorization->isAllowed('Licentia_Panda::tags_save')) {
            $this->getToolbar()
                 ->addChild(
                     'save-split-button',
    ...
    public function getHeaderText()
    {
        ...
        if ($this->registry->registry('panda_tag')
                           ->getId()) {
            return __(
                "Edit Tag '%1'",
                $this->escapeHtml(
                    $this->registry->registry('panda_tag')
                                   ->getName()
                )
            );
        } else {
