...
class Email extends AbstractModel
{
    ...
    public function add()
    {
        ...
        $template = $this->templatesFactory->create()->load($params['template_id']);
        $parentId = '';
        $data = [];
        $data['chain_id'] = $this->getData('chain_id');
        $data['autoresponder_id'] = $params['autoresponder_id'];
        $data['name'] = "Send Email: " . $template->getName();
    ...
    public function render()
    {
        ...
        $select = "<select name='template_id'>";

        foreach ($templates as $template) {
            $selected = '';
            if ($this->getData('extra_data/template_id') == $template->getId()) {
                $selected = ' selected="select" ';
            }

            $select .= "<option $selected value='{$template->getId()}'>{$template->getName()}</option>";
        }
        ...
        <input type="text" value="{$this->getData(
            'extra_data/subject'
        )}" class="required" name="subject" placeholder="{$t('Message Subject')}">
...
EOL;
    ...
    public function run(
        ...
        $template = $this->templatesFactory->create()->load($extraData['template_id']);
        ...
        $data['message'] = $template->getMessage();
        $data['sender_id'] = $extraData['sender_id'];
        $data['recurring'] = '0';
        $data['auto'] = '1';
        $data['type'] = 'email';
        $data['template_id'] = $template->getId();
        ...
        $data['autologin'] = $autoresponder->getAutologin();
        $data['global_template_id'] = $template->getData('global_template_id');
        $data['track'] = $autoresponder->getTrack();
